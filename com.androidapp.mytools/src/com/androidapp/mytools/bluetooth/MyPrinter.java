...
public class MyPrinter implements Runnable {
    ...
    class C00031 extends Thread {
        ...
        public void run() {
            while (MyPrinter.this.bta.getState() != BluetoothAdapter.STATE_ON) {
                try {
                    Thread.sleep(500);
    ...
    class C00042 extends Thread {
        ...
        public void run() {
            ...
            while (MyPrinter.this.bta.getState() != BluetoothAdapter.STATE_OFF) {
                try {
                    Thread.sleep(500);
                } catch (InterruptedException e) {
                    e.printStackTrace();
    ...
    class C00053 implements Runnable {
        ...
        public void run() {
            if (!MyPrinter.this.alwaysOn) {
                while (MyPrinter.this.bta.getState() == BluetoothAdapter.STATE_TURNING_ON) {
                    try {
                        Thread.sleep(500);
                    } catch (InterruptedException e) {
                        e.printStackTrace();
    ...
    class C00064 implements Runnable {
        ...
        public void run() {
            Toast.makeText(MyPrinter.this.context, "Please turnOn bluetooth printer..", Toast.LENGTH_LONG).show();
        }
    }
...
*/
    ...
    public void turnOnBT() {
        ...
        if (this.bta.getState() == BluetoothAdapter.STATE_TURNING_OFF) {
            new C00042().start();
        } else if (this.bta.getState() == BluetoothAdapter.STATE_OFF) {
            this.bta.enable();
        }
    }
    ...
    public void run() {
        while (true) {
            if (this.printQue.isEmpty()) {
                ...
                if (!this.alwaysOn && (this.bta.getState() == BluetoothAdapter.STATE_OFF || this.bta.getState() == BluetoothAdapter.STATE_TURNING_OFF)) {
                    initBluetooth();
                }
    ...
    public boolean isTurnedOn() {
        if (this.bta.getState() == BluetoothAdapter.STATE_ON) {
            return true;
        }
        return false;
    }
